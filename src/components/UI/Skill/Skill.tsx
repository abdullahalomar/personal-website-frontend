...
import Marquee from "react-marquee-slider"; // Adjust import if incorrect
import Image from "next/image";
...
const Motion: React.FC<MotionProps> = ({
  ...
  // Implement Motion logic if library doesn't support it natively
...
const Skill = () => {
  ...
  return (
    <div>
      <div className="text-center mb-16">
        <p className="text-2xl text-primary">My Skills</p>
        <h1 className="text-5xl font-bold">Growing Over Time</h1>
      </div>

      <div>
        <div style={{ height: "500px" }}>
          <Marquee velocity={12} resetAfterTries={200} scatterRandomly={true}>
            {images.map((src, index) => (
              <Motion
                key={`child-${index}`}
                initDeg={randomIntFromInterval(0, 360)}
